...
def get_example_app_defs(image: str, scheduler: SchedulerBackend) -> List[AppDef]:
    app_defs: List[AppDef] = [
        _get_cv_app_def(image, scheduler),
        _get_dist_cifar_app_def(image, scheduler),
        _get_dp_app_def(image, scheduler),
    ]
    return [app_def for app_def in app_defs if app_def]


def _get_cv_app_def(
        image: str, scheduler: SchedulerBackend
) -> Optional[AppDef]:
